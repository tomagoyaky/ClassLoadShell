 ...
 * to LOGD format chars or an ASCII filler ('.' or '?').
 *
...
void dumpFileHeader(const DexFile* pDexFile) {
	...
	if (pOptHeader != NULL) {
		LOGD("Optimized DEX file header:\n");

		asciify(sanitized, pOptHeader->magic, sizeof(pOptHeader->magic));
		LOGD("magic               : '%s'\n", sanitized);
		LOGD("dex_offset          : %d (0x%06x)\n", pOptHeader->dexOffset,
				pOptHeader->dexOffset);
		LOGD("dex_length          : %d\n", pOptHeader->dexLength);
		LOGD("deps_offset         : %d (0x%06x)\n", pOptHeader->depsOffset,
				pOptHeader->depsOffset);
		LOGD("deps_length         : %d\n", pOptHeader->depsLength);
		LOGD("opt_offset          : %d (0x%06x)\n", pOptHeader->optOffset,
				pOptHeader->optOffset);
	...
	LOGD("magic               : '%s'\n", sanitized);
	LOGD("checksum            : %08x\n", pHeader->checksum);
	LOGD("signature           : %02x%02x...%02x%02x\n", pHeader->signature[0],
			pHeader->signature[1], pHeader->signature[kSHA1DigestLen - 2],
			pHeader->signature[kSHA1DigestLen - 1]);
	LOGD("file_size           : %d\n", pHeader->fileSize);
	LOGD("header_size         : %d\n", pHeader->headerSize);
	LOGD("link_size           : %d\n", pHeader->linkSize);
	LOGD("link_off            : %d (0x%06x)\n", pHeader->linkOff,
			pHeader->linkOff);
	LOGD("string_ids_size     : %d\n", pHeader->stringIdsSize);
	LOGD("string_ids_off      : %d (0x%06x)\n", pHeader->stringIdsOff,
			pHeader->stringIdsOff);
	LOGD("type_ids_size       : %d\n", pHeader->typeIdsSize);
	LOGD("type_ids_off        : %d (0x%06x)\n", pHeader->typeIdsOff,
			pHeader->typeIdsOff);
	LOGD("proto_ids_size       : %d\n", pHeader->protoIdsSize);
	LOGD("proto_ids_off        : %d (0x%06x)\n", pHeader->protoIdsOff,
			pHeader->protoIdsOff);
	LOGD("field_ids_size      : %d\n", pHeader->fieldIdsSize);
	LOGD("field_ids_off       : %d (0x%06x)\n", pHeader->fieldIdsOff,
			pHeader->fieldIdsOff);
	LOGD("method_ids_size     : %d\n", pHeader->methodIdsSize);
	LOGD("method_ids_off      : %d (0x%06x)\n", pHeader->methodIdsOff,
			pHeader->methodIdsOff);
	LOGD("class_defs_size     : %d\n", pHeader->classDefsSize);
	LOGD("class_defs_off      : %d (0x%06x)\n", pHeader->classDefsOff,
			pHeader->classDefsOff);
	LOGD("data_size           : %d\n", pHeader->dataSize);
	LOGD("data_off            : %d (0x%06x)\n", pHeader->dataOff,
			pHeader->dataOff);
	LOGD("\n");
}
...
void dumpOptDirectory(const DexFile* pDexFile) {
	...
	LOGD("OPT section contents:\n");

	const u4* pOpt = (const u4*) ((u1*) pOptHeader + pOptHeader->optOffset);

	if (*pOpt == 0) {
		LOGD("(1.0 format, only class lookup table is present)\n\n");
		return;
	}
	...
	while (*pOpt != kDexChunkEnd) {
		...
		LOGD("Chunk %08x (%c%c%c%c) - %s (%d bytes)\n", *pOpt, *pOpt >> 24,
				(char )(*pOpt >> 16), (char )(*pOpt >> 8), (char )*pOpt,
				verboseStr, size);
...
void dumpClassDef(DexFile* pDexFile, int idx) {
	...
	if (pClassData == NULL) {
		LOGI("Trouble reading class data\n");
		return;
	}

	LOGD("Class #%d header:\n", idx);
	LOGD("class_idx           : %d\n", pClassDef->classIdx);
	LOGD("access_flags        : %d (0x%04x)\n", pClassDef->accessFlags,
			pClassDef->accessFlags);
	LOGD("superclass_idx      : %d\n", pClassDef->superclassIdx);
	LOGD("interfaces_off      : %d (0x%06x)\n", pClassDef->interfacesOff,
			pClassDef->interfacesOff);
	LOGD("source_file_idx     : %d\n", pClassDef->sourceFileIdx);
	LOGD("annotations_off     : %d (0x%06x)\n", pClassDef->annotationsOff,
			pClassDef->annotationsOff);
	LOGD("class_data_off      : %d (0x%06x)\n", pClassDef->classDataOff,
			pClassDef->classDataOff);
	LOGD("static_fields_size  : %d\n", pClassData->header.staticFieldsSize);
	LOGD("instance_fields_size: %d\n", pClassData->header.instanceFieldsSize);
	LOGD("direct_methods_size : %d\n", pClassData->header.directMethodsSize);
	LOGD("virtual_methods_size: %d\n", pClassData->header.virtualMethodsSize);
	LOGD("\n");
...
void dumpPositions(DexFile* pDexFile, const DexCode* pCode,
	...
	const char *classDescriptor = dexStringByTypeIdx(pDexFile,
			pMethodId->classIdx);
...
static void dumpLocalsCb(void *cnxt, u2 reg, u4 startAddress, u4 endAddress,
		const char *name, const char *descriptor, const char *signature) {
	LOGD("        0x%04x - 0x%04x reg=%d %s %s %s\n", startAddress, endAddress,
			reg, name, descriptor, signature);
}
 ...
 */
bool getMethodInfo(DexFile* pDexFile, u4 methodIdx,
		FieldMethodInfo* pMethInfo) {
	...
	pMethInfo->classDescriptor = dexStringByTypeIdx(pDexFile,
			pMethodId->classIdx);
	return true;
}
...
bool getFieldInfo(DexFile* pDexFile, u4 fieldIdx, FieldMethodInfo* pFieldInfo) {
	...
	pFieldInfo->classDescriptor = dexStringByTypeIdx(pDexFile,
			pFieldId->classIdx);
	return true;
}
 ...
 * needs to be free()d.
 */
static char* indexString(DexFile* pDexFile, const DecodedInstruction* pDecInsn,
	...
	case kIndexVaries:
		 ...
		 * to get fancy here.
		 */
		outSize = snprintf(buf, bufSize, "<index-varies> // thing@%0*x", width,
				index);
		break;
	case kIndexTypeRef:
		if (index < pDexFile->pHeader->typeIdsSize) {
			...
			outSize = snprintf(buf, bufSize, "<type?> // type@%0*x", width,
					index);
		}
		break;
	case kIndexStringRef:
		if (index < pDexFile->pHeader->stringIdsSize) {
			...
			outSize = snprintf(buf, bufSize, "<string?> // string@%0*x", width,
					index);
		}
		break;
	case kIndexMethodRef: {
		...
		if (getMethodInfo(pDexFile, index, &methInfo)) {
			...
			outSize = snprintf(buf, bufSize, "<method?> // method@%0*x", width,
					index);
		}
	}
		break;
	case kIndexFieldRef: {
		...
		if (getFieldInfo(pDexFile, index, &fieldInfo)) {
			...
			outSize = snprintf(buf, bufSize, "<field?> // field@%0*x", width,
					index);
		}
	}
		break;
	case kIndexInlineMethod:
		outSize = snprintf(buf, bufSize, "[%0*x] // inline #%0*x", width, index,
				width, index);
		break;
	case kIndexVtableOffset:
		outSize = snprintf(buf, bufSize, "[%0*x] // vtable #%0*x", width, index,
				width, index);
		break;
...
void dumpInstruction(DexFile* pDexFile, const DexCode* pCode, int insnIdx,
	...
	LOGD("%06x:", ((u1* )insns - pDexFile->baseAddr) + insnIdx * 2);
	for (i = 0; i < 8; i++) {
		if (i < insnWidth) {
			if (i == 7) {
				LOGD(" ... ");
				...
				LOGD(" %02x%02x", bytePtr[0], bytePtr[1]);
			}
		} else {
			fputs("     ", stdout);
		}
	...
	if (pDecInsn->opcode == OP_NOP) {
		...
		if (instr == kPackedSwitchSignature) {
			LOGD("|%04x: packed-switch-data (%d units)", insnIdx, insnWidth);
		} else if (instr == kSparseSwitchSignature) {
			LOGD("|%04x: sparse-switch-data (%d units)", insnIdx, insnWidth);
		} else if (instr == kArrayDataSignature) {
			LOGD("|%04x: array-data (%d units)", insnIdx, insnWidth);
		} else {
			LOGD("|%04x: nop // spacer", insnIdx);
		}
	} else {
		LOGD("|%04x: %s", insnIdx, dexGetOpcodeName(pDecInsn->opcode));
	}

	if (pDecInsn->indexType != kIndexNone) {
		indexBuf = indexString(pDexFile, pDecInsn, indexBufChars,
				sizeof(indexBufChars));
	}
	...
	case kFmt12x:        // op vA, vB
		LOGD(" v%d, v%d", pDecInsn->vA, pDecInsn->vB);
		break;
	case kFmt11n:        // op vA, #+B
		LOGD(" v%d, #int %d // #%x", pDecInsn->vA, (s4 )pDecInsn->vB,
				(u1 )pDecInsn->vB);
		break;
	case kFmt11x:        // op vAA
		LOGD(" v%d", pDecInsn->vA);
		break;
	case kFmt10t:        // op +AA
	case kFmt20t:        // op +AAAA
	{
		s4 targ = (s4) pDecInsn->vA;
		LOGD(" %04x // %c%04x", insnIdx + targ, (targ < 0) ? '-' : '+',
				(targ < 0) ? -targ : targ);
	}
		break;
	...
	case kFmt21t:        // op vAA, +BBBB
	{
		...
		LOGD(" v%d, %04x // %c%04x", pDecInsn->vA, insnIdx + targ,
	...
	case kFmt21h:        // op vAA, #+BBBB0000[00000000]
		...
		if (pDecInsn->opcode == OP_CONST_HIGH16) {
			...
			LOGD(" v%d, #int %d // #%x", pDecInsn->vA, value,
					(u2 )pDecInsn->vB);
		} else {
			s8 value = ((s8) pDecInsn->vB) << 48;
			LOGD(" v%d, #long %lld // #%x", pDecInsn->vA, value,
					(u2 )pDecInsn->vB);
		}
		break;
	case kFmt21c:        // op vAA, thing@BBBB
	case kFmt31c:        // op vAA, thing@BBBBBBBB
		LOGD(" v%d, %s", pDecInsn->vA, indexBuf);
		break;
	case kFmt23x:        // op vAA, vBB, vCC
		LOGD(" v%d, v%d, v%d", pDecInsn->vA, pDecInsn->vB, pDecInsn->vC);
		break;
	case kFmt22b:        // op vAA, vBB, #+CC
		LOGD(" v%d, v%d, #int %d // #%02x", pDecInsn->vA, pDecInsn->vB,
				(s4 )pDecInsn->vC, (u1 )pDecInsn->vC);
		break;
	case kFmt22t:        // op vA, vB, +CCCC
	{
		s4 targ = (s4) pDecInsn->vC;
		LOGD(" v%d, v%d, %04x // %c%04x", pDecInsn->vA, pDecInsn->vB,
				insnIdx + targ, (targ < 0) ? '-' : '+',
				(targ < 0) ? -targ : targ);
	}
		break;
	case kFmt22s:        // op vA, vB, #+CCCC
		LOGD(" v%d, v%d, #int %d // #%04x", pDecInsn->vA, pDecInsn->vB,
				(s4 )pDecInsn->vC, (u2 )pDecInsn->vC);
		break;
	case kFmt22c:        // op vA, vB, thing@CCCC
	case kFmt22cs:       // [opt] op vA, vB, field offset CCCC
		LOGD(" v%d, v%d, %s", pDecInsn->vA, pDecInsn->vB, indexBuf);
		break;
	case kFmt30t:
		LOGD(" #%08x", pDecInsn->vA);
		break;
	case kFmt31i:        // op vAA, #+BBBBBBBB
	{
		...
		conv.i = pDecInsn->vB;
		LOGD(" v%d, #float %f // #%08x", pDecInsn->vA, conv.f, pDecInsn->vB);
	}
		break;
	case kFmt31t:       // op vAA, offset +BBBBBBBB
		LOGD(" v%d, %08x // +%08x", pDecInsn->vA, insnIdx + pDecInsn->vB,
				pDecInsn->vB);
		break;
	...
	case kFmt51l:        // op vAA, #+BBBBBBBBBBBBBBBB
	{
		...
		LOGD(" v%d, #double %f // #%016llx", pDecInsn->vA, conv.d,
				pDecInsn->vB_wide);
	}
		break;
...
void dumpCode(DexFile* pDexFile, const DexMethod* pDexMethod) {
	...
	LOGD("      ins           : %d\n", pCode->insSize);
	LOGD("      outs          : %d\n", pCode->outsSize);
	LOGD("      insns size    : %d 16-bit code units\n", pCode->insnsSize);
...
void dumpMethod(DexFile* pDexFile, const DexMethod* pDexMethod, int i) {
	...
	if (gOptions.outputFormat == OUTPUT_PLAIN) {
		...
		LOGD("      name          : '%s'\n", name);
		LOGD("      type          : '%s'\n", typeDescriptor);
		LOGD("      access        : 0x%04x (%s)\n", pDexMethod->accessFlags,
				accessStr);
		...
		if (constructor) {
			...
			if (returnType == NULL) {
				LOGI("bad method type descriptor '%s'\n", typeDescriptor);
				goto bail;
			}
			...
			bool isSync = (pDexMethod->accessFlags & ACC_SYNCHRONIZED) != 0
					|| (pDexMethod->accessFlags & ACC_DECLARED_SYNCHRONIZED)
							!= 0;
			LOGD(" synchronized=%s\n", quotedBool(isSync));
		}
		...
		while (*base != ')') {
			...
			if (*base == 'L') {
				...
				if (strchr("ZBCSIFJD", *base) == NULL) {
					LOGI("ERROR: bad method signature '%s'\n", base);
					goto bail;
				}
				*cp++ = *base++;
			}
			...
			*cp++ = '\0';
			...
			LOGD("<parameter name=\"arg%d\" type=\"%s\">\n</parameter>\n",
					argNum++, tmp);
			free(tmp);
		}
...
void dumpClass(DexFile* pDexFile, int idx, char** pLastPackage) {
	...
	if (pClassData == NULL) {
		LOGD("Trouble reading class data (#%d)\n", idx);
		goto bail;
	}
	...
	if (pClassDef->superclassIdx == kDexNoIndex) {
		...
		superclassDescriptor = dexStringByTypeIdx(pDexFile,
				pClassDef->superclassIdx);
	}

	if (gOptions.outputFormat == OUTPUT_PLAIN) {
		LOGD("Class #%d            -\n", idx);
		LOGD("  Class descriptor  : '%s'\n", classDescriptor);
		LOGD("  Access flags      : 0x%04x (%s)\n", pClassDef->accessFlags,
				accessStr);
...
void dumpDifferentialCompressedMap(const u1** pData) {
	...
	LOGD("        (differential compression %d -> %d [%d -> %d])\n", origLen,
			compLen, (addrWidth + regWidth) * numEntries, compressedLen);
...
void dumpRegisterMaps(DexFile* pDexFile) {
	...
	LOGD("RMAP begins at offset 0x%07x\n", baseFileOffset);
	LOGD("Maps for %d classes\n", numClasses);
...
int process(const char* fileName) {
	...
	if (pDexFile == NULL) {
		LOGI("ERROR: DEX parse failed\n");
		goto bail;
	}

	if (gOptions.checksumOnly) {
		LOGD("Checksum verified\n");
	} else {
 ...
 */
void usage(void) {
	LOGI("Copyright (C) 2007 The Android Open Source Project\n\n");
	fprintf(stderr,
			"%s: [-c] [-d] [-f] [-h] [-i] [-l layout] [-m] [-t tempfile] dexfile...\n",
			gProgName);
	LOGI("\n");
	LOGI(" -c : verify checksum and exit\n");
	LOGI(" -d : disassemble code sections\n");
	LOGI(" -f : display summary information from file header\n");
	LOGI(" -h : display file header details\n");
	LOGI(" -i : ignore checksum failures\n");
	LOGI(" -l : output layout, either 'plain' or 'xml'\n");
	LOGI(" -m : dump register maps (and nothing else)\n");
	LOGI(" -t : temp file name (defaults to /sdcard/dex-temp-*)\n");
}
...
int main(int argc, char* const argv[]) {
	...
	if (optind == argc) {
		LOGI("%s: no file specified\n", gProgName);
		wantUsage = true;
	}

	if (gOptions.checksumOnly && gOptions.ignoreBadChecksum) {
		LOGI("Can't specify both -c and -i\n");
		wantUsage = true;
	}
